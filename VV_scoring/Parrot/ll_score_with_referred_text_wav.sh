...
compile-train-graphs --read-disambig-syms=$lang_dir/phones/disambig.int \
  ...
  ll-score-with-referred-text-wav --transition-scale=1.0 --acoustic-scale=0.1 --self-loop-scale=0.1 --frames-per-chunk=50 --extra-left-context=0 --extra-right-context=0 --extra-left-context-initial=-1 --extra-right-context-final=-1 --use-gpu=no --beam=10 --retry-beam=40 $nnet3_dir/final.mdl ark:- ark:$data_dir/feats.ark ark:tmp.ark

...
int main(int argc, char *argv[]) {
  try {
    ...
    BaseFloat intra_ratio = 0.0;  // ratio for the intra distance in the loss
    ...
    po.Register("intra-ratio", &intra_ratio, "ratio for the intra distance in the loss");
    ...
    for (; !example_reader.Done(); example_reader.Next())
      trainer.TrainIntra(example_reader.Value(), intra_ratio);

...
/// training of DNNs with Natural Gradient and Parameter Averaging" (ICLR
/// workshop, 2015) by Daniel Povey, Xiaohui Zhang and Sanjeev Khudanpur.
/// Linear_params plus one before using, we get PlusOneNaturalGradientAffineComponent.
class PlusOneNaturalGradientAffineComponent: public AffineComponent {
 public:
  virtual std::string Type() const { return "PlusOneNaturalGradientAffineComponent"; }
  virtual void Read(std::istream &is, bool binary);
  virtual void Write(std::ostream &os, bool binary) const;
  void Init(int32 input_dim, int32 output_dim,
            BaseFloat param_stddev, BaseFloat bias_stddev, BaseFloat bias_mean,
  ...
  void Init(int32 rank_in, int32 rank_out, int32 update_period,
            ...
            std::string matrix_filename);
  // this constructor does not really initialize, use Init() or Read().
  PlusOneNaturalGradientAffineComponent();
  virtual void Resize(int32 input_dim, int32 output_dim);
  virtual void InitFromConfig(ConfigLine *cfl);
  virtual std::string Info() const;
  virtual Component* Copy() const;
  ...
  virtual void Propagate(const ComponentPrecomputedIndexes *indexes,
                         ...
                         CuMatrixBase<BaseFloat> *out) const;
  // copy constructor
  explicit PlusOneNaturalGradientAffineComponent(
      const PlusOneNaturalGradientAffineComponent &other);
  virtual void ZeroStats();

 private:
  // disallow assignment operator.
  PlusOneNaturalGradientAffineComponent &operator= (
      const PlusOneNaturalGradientAffineComponent&);
...
class OppositeComponent: public NonlinearComponent {
 public:
  explicit OppositeComponent(const OppositeComponent &other): NonlinearComponent(other) { }
  OppositeComponent() { }
  virtual std::string Type() const { return "OppositeComponent"; }
  virtual int32 Properties() const {
    return kSimpleComponent|kLinearInInput|kPropagateInPlace;
  }
  virtual Component* Copy() const { return new OppositeComponent(*this); }
  virtual void Propagate(const ComponentPrecomputedIndexes *indexes,
                         const CuMatrixBase<BaseFloat> &in,
                         CuMatrixBase<BaseFloat> *out) const;
  virtual void Backprop(const std::string &debug_info,
                        ...
                        CuMatrixBase<BaseFloat> *in_deriv) const;
 private:
  OppositeComponent &operator = (const OppositeComponent &other); // Disallow.
};

// LeakyComponent: output = 1 - input. 
class LeakyComponent: public NonlinearComponent {
 public:
  explicit LeakyComponent(const LeakyComponent &other): NonlinearComponent(other) { }
  LeakyComponent() { }
  virtual std::string Type() const { return "LeakyComponent"; }
  virtual int32 Properties() const {
    return kSimpleComponent|kLinearInInput|kPropagateInPlace;
  }
  virtual Component* Copy() const { return new LeakyComponent(*this); }
  virtual void Propagate(const ComponentPrecomputedIndexes *indexes,
                         const CuMatrixBase<BaseFloat> &in,
                         CuMatrixBase<BaseFloat> *out) const;
  virtual void Backprop(const std::string &debug_info,

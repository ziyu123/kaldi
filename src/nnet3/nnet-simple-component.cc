...
void NoBpComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
                                 const CuMatrixBase<BaseFloat> &in,
                                 CuMatrixBase<BaseFloat> *out) const {
  out->CopyFromMat(in);
}

void NoBpComponent::Backprop(const std::string &debug_info,
                             const ComponentPrecomputedIndexes *indexes,
                             const CuMatrixBase<BaseFloat> &,
                             const CuMatrixBase<BaseFloat> &,
                             const CuMatrixBase<BaseFloat> &out_deriv,
...
PlusOneNaturalGradientAffineComponent::PlusOneNaturalGradientAffineComponent():
    max_change_per_sample_(0.0),
    update_count_(0.0), active_scaling_count_(0.0),
    max_change_scale_stats_(0.0) { }

// virtual
void PlusOneNaturalGradientAffineComponent::Resize(
    int32 input_dim, int32 output_dim) {
  KALDI_ASSERT(input_dim > 1 && output_dim > 1);
  if (rank_in_ >= input_dim) rank_in_ = input_dim - 1;
  if (rank_out_ >= output_dim) rank_out_ = output_dim - 1;
...
void PlusOneNaturalGradientAffineComponent::Read(std::istream &is, bool binary) {
  ReadUpdatableCommon(is, binary);  // Read the opening tag and learning rate
  ExpectToken(is, binary, "<LinearParams>");
  linear_params_.Read(is, binary);
  ExpectToken(is, binary, "<BiasParams>");
  ...
  if (token != "<PlusOneNaturalGradientAffineComponent>" &&
      token != "</PlusOneNaturalGradientAffineComponent>")
    KALDI_ERR << "Expected <PlusOneNaturalGradientAffineComponent> or "
              << "</PlusOneNaturalGradientAffineComponent>, got " << token;
  SetNaturalGradientConfigs();
}

void PlusOneNaturalGradientAffineComponent::InitFromConfig(ConfigLine *cfl) {
  bool ok = true;
  std::string matrix_filename;
  BaseFloat num_samples_history = 2000.0, alpha = 4.0,
      max_change_per_sample = 0.0;
...
void PlusOneNaturalGradientAffineComponent::SetNaturalGradientConfigs() {
  preconditioner_in_.SetRank(rank_in_);
  preconditioner_in_.SetNumSamplesHistory(num_samples_history_);
  preconditioner_in_.SetAlpha(alpha_);
  preconditioner_in_.SetUpdatePeriod(update_period_);
...
void PlusOneNaturalGradientAffineComponent::Init(
    int32 rank_in, int32 rank_out,
    int32 update_period, BaseFloat num_samples_history, BaseFloat alpha,
    BaseFloat max_change_per_sample,
    std::string matrix_filename) {
...
void PlusOneNaturalGradientAffineComponent::Init(
  ...
  if (max_change_per_sample > 0.0)
    KALDI_WARN << "You are setting a positive max_change_per_sample for "
               << "PlusOneNaturalGradientAffineComponent. But the per-component "
               << "gradient clipping mechansim has been removed. Instead it's currently "
               << "done at the whole model level.";
  max_change_per_sample_ = max_change_per_sample;
  is_gradient_ = false;  // not configurable; there's no reason you'd want this
...
void PlusOneNaturalGradientAffineComponent::Write(std::ostream &os,
                                           bool binary) const {
  WriteUpdatableCommon(os, binary);  // Write the opening tag and learning rate
  WriteToken(os, binary, "<LinearParams>");
  linear_params_.Write(os, binary);
  ...
  WriteToken(os, binary, "<MaxChangeScaleStats>");
  WriteBasicType(os, binary, max_change_scale_stats_);
  WriteToken(os, binary, "</PlusOneNaturalGradientAffineComponent>");
}

std::string PlusOneNaturalGradientAffineComponent::Info() const {
  std::ostringstream stream;
  stream << UpdatableComponent::Info();
  PrintParameterStats(stream, "linear-params", linear_params_);
  PrintParameterStats(stream, "bias", bias_params_, true);
...
void PlusOneNaturalGradientAffineComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
                                const CuMatrixBase<BaseFloat> &in,
                                 CuMatrixBase<BaseFloat> *out) const {
  ...
  out->CopyRowsFromVec(bias_params_); // copies bias_params_ to each row
  // of *out.
  CuMatrix<BaseFloat> linear_params_temp(linear_params_);
  linear_params_temp.Add(1.0);
  out->AddMatMat(1.0, in, kNoTrans, linear_params_temp, kTrans, 1.0);
}


Component* PlusOneNaturalGradientAffineComponent::Copy() const {
  return new PlusOneNaturalGradientAffineComponent(*this);
}

PlusOneNaturalGradientAffineComponent::PlusOneNaturalGradientAffineComponent(
    const PlusOneNaturalGradientAffineComponent &other):
    AffineComponent(other),
    rank_in_(other.rank_in_),
    rank_out_(other.rank_out_),
    update_period_(other.update_period_),
...
void PlusOneNaturalGradientAffineComponent::Update(
    const std::string &debug_info,
    const CuMatrixBase<BaseFloat> &in_value,
    const CuMatrixBase<BaseFloat> &out_deriv) {
  CuMatrix<BaseFloat> in_value_temp;
...
void PlusOneNaturalGradientAffineComponent::ZeroStats()  {
  update_count_ = 0.0;
  max_change_scale_stats_ = 0.0;
  active_scaling_count_ = 0.0;
}

void PlusOneNaturalGradientAffineComponent::Scale(BaseFloat scale) {
  update_count_ *= scale;
  max_change_scale_stats_ *= scale;
  active_scaling_count_ *= scale;
  linear_params_.Scale(scale);
...
void PlusOneNaturalGradientAffineComponent::Add(BaseFloat alpha, const Component &other_in) {
  const PlusOneNaturalGradientAffineComponent *other =
      dynamic_cast<const PlusOneNaturalGradientAffineComponent*>(&other_in);
  KALDI_ASSERT(other != NULL);
  update_count_ += alpha * other->update_count_;
  max_change_scale_stats_ += alpha * other->max_change_scale_stats_;
  active_scaling_count_ += alpha * other->active_scaling_count_;

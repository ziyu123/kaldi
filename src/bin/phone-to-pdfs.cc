...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "Usage:  phone-to-pdfs  [options] <model> <alignments-rspecifier> <pdfs-wspecifier>\n"
        "e.g.: \n"
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      bool ans = GetPdfsForPhones(trans_model, alignment, &pdfs); 
      if (ans)
        KALDI_LOG << "Sharing pdfs"; 
      writer.Write(key, pdfs);
      num_done++;
    }
    KALDI_LOG << "Converted " << num_done << " phones to pdf sequences.";

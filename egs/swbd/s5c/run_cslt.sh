...
fisher_dirs=/nfs/corpus0/data/corpora/fisher/transcripts
...
for x in train eval2000; do
  steps/make_mfcc.sh --nj 32 --cmd "$train_cmd" \
    data/$x data/$x/mfcc/log data/$x/mfcc
  steps/compute_cmvn_stats.sh data/$x data/$x/mfcc/log data/$x/mfcc
  utils/fix_data_dir.sh data/$x
done

for x in train eval2000; do
  steps/make_fbank.sh --nj 32 --cmd "$train_cmd" \

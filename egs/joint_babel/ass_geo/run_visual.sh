...
for mdl in mdl_from_ass_geo/*; do
  for utt in `seq 4`; do
    nnet3-compute --use-gpu=no --extra-left-context=40 --frames-per-chunk=20 $mdl scp:data_fbank/test${utt}_100/feats.scp ark,t:mdl_from_ass_geo/${mdl}_${utt}
  done
done
...
common_egs_dir=exp/nnet3/lstm/egs
dir=exp/nnet3/lstm_lr0.00001
...
if [ $stage -le 8 ]; then
  steps/nnet3/lstm/train.sh --stage $train_stage \
    ...
    data_fbank/train data/lang exp/tri4b_ali $dir  || exit 1;
fi

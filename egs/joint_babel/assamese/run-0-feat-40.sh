...
echo "mfcc.conf should support 40 dim."

if true; then


# make fbank features
mkdir -p data_fbank_40
for x in train test; do
 cp -r data/$x data_fbank_40/$x
 steps/make_fbank.sh --nj 15 --cmd "$train_cmd" data_fbank_40/$x || exit 1;
 steps/compute_cmvn_stats.sh data_fbank_40/$x || exit 1;
done

    ...
    for i in range(args.num_models):
        ...
        for line_num in range(1, len(lines) + 1):
            ...
            if mat_obj is not None:
                if str(mat_obj.groups()[0]) == 'output':
                    this_loss = float(mat_obj.groups()[1])
                if str(mat_obj.groups()[0]) == 'output_speech':
                    this_loss_plus = float(mat_obj.groups()[1])
                break;
        loss.append(this_loss);
        loss_plus.append(this_loss_plus);
    max_index = loss.index(max(loss))
    max_index_plus = loss_plus.index(max(loss_plus))
    accepted_models = []
    for i in range(args.num_models):
        if (loss[max_index] - loss[i]) <= args.difference_threshold or (loss_plus[max_index_plus] - loss_plus[i]) <= args.difference_threshold:

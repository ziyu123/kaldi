...
# Config files set by hand, num_hidden_layers in vars > 0 will be ok, as it will be set in the script
for f in $dir/configs/init.config $dir/configs/layer${num_lstm_layers}.config $dir/configs/vars; do
...
if [ $stage -le -5 ]; then
  echo "$0: neural net configs already set by hand";
...
if [ $stage -le -1 ]; then
  ...
  # Add rest layers here at once, sometimes several layers in one config file, thus a module
  x=2

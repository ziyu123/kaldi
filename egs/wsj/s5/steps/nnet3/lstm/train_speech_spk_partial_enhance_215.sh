...
for f in $dir/base.raw $dir/configs/last_hid.config $dir/configs/vars; do
  [ ! -f $f ] && echo "$0: no such file $f, config file set by hand, from baseline " && exit 1;
done
if [ -z "$egs_dir" ]; then
  echo "$0: egs combined by hand." && exit 1;
fi
...
if [ $stage -le -1 ]; then
  # random last hidden layer
  $cmd $dir/log/random_last_layer.log \
       nnet3-init --srand=-3 $dir/base.raw $dir/configs/last_hid.config $dir/0.raw || exit 1;

  # Convert to .mdl, train the transitions, set the priors.
  $cmd $dir/log/init_mdl.log \
    nnet3-am-init $alidir/final.mdl $dir/0.raw - \| \
    nnet3-am-train-transitions-special - "ark:gunzip -c $alidir/ali.*.gz|" $dir/0.mdl || exit 1;
fi
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    ( # this sub-shell is so that when we "wait" below,
      ...
      for n in $(seq $this_num_jobs); do
        ...
        gpu_used=$[$n%max_gpu_num]
        if [ $gpu_used -eq 0 ] && [ $n -lt $this_num_jobs ]; then
          wait
        fi
    ...
    models_to_average=$(steps/nnet3/get_successful_models_partial.py $this_num_jobs $dir/log/train.$x.%.log)
    nnets_list=
...
if [ $stage -le $num_iters ]; then
  ...
  $cmd $combine_queue_opt $dir/log/combine.log \
    nnet3-combine-special --num-iters=40 \
...
if [ $stage -le $[$num_iters+1] ]; then
  ...
  num_jobs_compute_prior=$max_gpu_num
  if [ $num_jobs_compute_prior -gt $num_archives ]; then egs_part=1;
  else egs_part=JOB; fi

  cur_egs_dir=exp/nnet3/partial_egs/egs_1_prior
  $cmd JOB=1:$num_jobs_compute_prior $prior_queue_opt $dir/log/get_post.$x.JOB.log \
    ...
    nnet3-compute-from-egs-special $prior_gpu_opt --apply-exp=true \

...
for f in $dir/configs/init.config $dir/configs/layer1.config $dir/configs/vars; do
  [ ! -f $f ] && echo "$0: no such file $f, config file set by hand, ivector as input" && exit 1;
done
...
if [ $stage -le -4 ] && [ -z "$egs_dir" ]; then
  ...
  steps/nnet3/get_egs_spk_pure_ivector_in.sh $egs_opts "${extra_opts[@]}" \
      --cmd "$cmd" $egs_opts \
      --stage $get_egs_stage \

...
if [ $stage -le $num_iters ]; then
  ...
  $gpu_cmd $combine_queue_opt $dir/log/combine_1.log \
    nnet3-combine --num-iters=40 \
       --enforce-sum-to-one=true --enforce-positive-weights=true \
       --verbose=3 "${nnets_list[@]}" "ark:nnet3-merge-egs --measure-output-frames=false --minibatch-size=$combine_num_chunk_per_minibatch ark:$cur_egs_dir/combine_1.egs ark:-|" \
    $dir/combined_1.raw || exit 1;

  # Compute the probability of the final, combined model with
  # the same subset we used for the previous compute_probs, as the
  # different subsets will lead to different probs.
  $cmd $dir/log/compute_prob_valid.final_1.log \
    nnet3-compute-prob $dir/combined_1.raw \
    "ark:nnet3-merge-egs --minibatch-size=256 ark:$cur_egs_dir/valid_diagnostic.egs ark:- |" &
  $cmd $dir/log/compute_prob_train.final_1.log \
    nnet3-compute-prob $dir/combined_1.raw \
    "ark:nnet3-merge-egs --minibatch-size=256 ark:$cur_egs_dir/train_diagnostic.egs ark:- |" &

  # for language 2
  # fisrst convert output2 to output, output to output1
  echo "convert output2 to output of models:"
  echo ${nnets_list[@]}
  sed -i 's/name=output /name=output1 /g' ${nnets_list[@]}
  sed -i 's/name=output2 /name=output /g' ${nnets_list[@]}
  #
  $gpu_cmd $combine_queue_opt $dir/log/combine_2.log \
    nnet3-combine --num-iters=40 \
       --enforce-sum-to-one=true --enforce-positive-weights=true \
       --verbose=3 "${nnets_list[@]}" "ark:nnet3-merge-egs --measure-output-frames=false --minibatch-size=$combine_num_chunk_per_minibatch ark:$cur_egs_dir/combine_2.egs ark:-|" \
    $dir/combined_2.raw || exit 1;
  # convert output back to output2, output1 to output
  echo "convert output back to output2 of those models and combined.raw."
  sed -i 's/name=output /name=output2 /g' ${nnets_list[@]}
  sed -i 's/name=output /name=output2 /g' $dir/combined_2.raw
  sed -i 's/name=output1 /name=output /g' ${nnets_list[@]}
  sed -i 's/name=output1 /name=output /g' $dir/combined_2.raw

  # Compute the probability of the final, combined model with
  # the same subset we used for the previous compute_probs, as the
  # different subsets will lead to different probs.
  $cmd $dir/log/compute_prob_valid.final_2.log \
    nnet3-compute-prob $dir/combined_2.raw \
    "ark:nnet3-merge-egs --minibatch-size=256 ark:$cur_egs_dir/valid_diagnostic.egs ark:- |" &
  $cmd $dir/log/compute_prob_train.final_2.log \
    nnet3-compute-prob $dir/combined_2.raw \
    "ark:nnet3-merge-egs --minibatch-size=256 ark:$cur_egs_dir/train_diagnostic.egs ark:- |" &
fi

# just get combined.raw
echo "------ Done combination ------"
wait;
echo "------ Done computing prob ------"
exit 0;

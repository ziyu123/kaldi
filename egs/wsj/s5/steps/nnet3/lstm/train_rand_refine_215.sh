...
# Check some files.
# Provide a base model by hand.
for f in $data/feats.scp $lang/L.fst $alidir/ali.1.gz $alidir/final.mdl $alidir/tree $dir/base.raw; do
  [ ! -f $f ] && echo "$0: no such file $f" && exit 1;
done

# Config file set by hand, num_hidden_layers in vars > 0 will be ok
for f in $dir/configs/last_hid.config $dir/configs/vars; do

...
train_stage=-10 #-10
affix=
common_egs_dir=exp/nnet3/lstm_config_ld5/egs

# LSTM options
splice_indexes="-2,-1,0,1,2 0 0 0 0 0"
lstm_delay=" -1 -1 -1 -1 -1 -1 "
label_delay=5
num_lstm_layers=6
cell_dim=1024
hidden_dim=1024
recurrent_projection_dim=512 #256
non_recurrent_projection_dim=0 #256
chunk_width=20
...
# For reading configs 'config/layer*.config', sometimes several layers in one config file, thus it's the number of modules
num_lstm_layers=1

dir=exp/nnet3/lstm-${num_lstm_layers}

if [ $stage -le 8 ]; then
  steps/nnet3/lstm/train_from_config.sh --stage $train_stage \
    ...
    data_fbank/train data_fbank/lang exp/tri4b_ali $dir  || exit 1;
fi
...
if [ $stage -le 9 ]; then
  ...
  for lm_suffix in tgpr; do
    ...
    for year in eval92 dev93 eval93 all; do
      (
      num_jobs=8
